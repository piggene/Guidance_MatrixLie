...
def get_ode_solver(cfg):
    ...
    elif name == 'SE3_RK_mk_guide':
        solver = SE3_RK4_MK_guide(**cfg)
    else:
...
class SE3_RK4_MK_guide:
    def __init__(self, num_steps):
        self.t = torch.linspace(0, 1, num_steps + 1)

    @torch.no_grad()
    def __call__(self, z, x_0, x_1, func):
        # Initialize
        t = self.t.to(z.device)
        dt = t[1:] - t[:-1]
        traj = x_0.new_zeros(x_0.shape[0:1] + t.shape + x_0.shape[1:])
        traj[:, 0] = x_0
        print("New Roation solving....")
        for n in range(len(t)-1):
            print("iteration :",n)
            # Get n-th values
            x_n = traj[:, n].contiguous()
            ...
            # Get vector (V_s)
            V_2 = func(z, x_1, t_n + (h / 2), x_hat_2)
            w_2 = V_2[:, :3]
            v_2 = V_2[:, 3:]
            ...
            # Get vector (V_s)
            V_3 = func(z, x_1, t_n + (h / 2), x_hat_3)
            w_3 = V_3[:, :3]
            v_3 = V_3[:, 3:]
