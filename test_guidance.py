...
def test(model, test_loader, device, visualizer, guide_type):
    ...
    pc_list_types = test_loader.dataset.pc_list_types
    ...
    # Iterate
    for i, (obj_type, obj_idxs_objs, pc_list_objs, mesh_list_objs, Ts_grasp_list_objs) in enumerate(zip(obj_types, obj_idxs_types, pc_list_types, mesh_list_types, Ts_grasp_list_types)):
        for j, (obj_idx, pc_rots, mesh_list_rots, Ts_grasp_rots_target) in enumerate(zip(obj_idxs_objs, pc_list_objs, mesh_list_objs, Ts_grasp_list_objs)):
            # Setup input
            pc_rots = torch.Tensor(pc_rots).to(device)
            ...
            for k, (mesh, Ts_grasp_pred, Ts_grasp_target) in enumerate(zip(mesh_list_rots, Ts_grasp_rots_pred, Ts_grasp_rots_target)):
                # Setup message
                msg = f"object type: {obj_type}, object index: {obj_idx}, rotation index: {k}, "
                ...
                # Get indices for sampling grasp poses for visualization
                idxs = torch.randperm(len(Ts_grasp_pred))[:NUM_GRASPS]

                # Add mesh and gripper to visualizer
                visualizer.add_mesh(mesh, row=1, col=k+1)
                visualizer.add_grippers(Ts_grasp_pred[idxs], color='grey', row=1, col=k+1)
...
def save_figure(logdir, visualizer):
    ...
    for idx_scene, label in enumerate(visualizer.labels):
        ...
        for idx_trace in range(num_subplots*(1+NUM_GRASPS)*idx_scene, num_subplots*(1+NUM_GRASPS)*(idx_scene+1)):
